...
pub async fn send_msg(stream: &mut tokio::io::BufWriter<tokio::net::tcp::OwnedWriteHalf>, msg: &ClientMsg) -> Result<()> {
...
pub async fn run_scenario(mode: &str, count: usize, msg_per_bot: usize) {
    ...
    for i in 0..count {
        ...
        let handle = tokio::spawn(async move {
            let result = match mode.as_str() {
                ...
                "spammer"  => spammer::run(i as u64, msg_per_bot).await,
    ...
    if mode == "normal" {
        ...
        assert_eq!(expected, received, "메시지 누락 발생! expected={expected} received={received}");
        info!("누락 없음 확인");
    }
}
